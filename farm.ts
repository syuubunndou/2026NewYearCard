...
class Notebook{
    ...
    BOARD_OVERLAY    : HTMLDivElement;
    HOUSE_IMG        : HTMLImageElement;
    CLOSE_BTN        : HTMLDivElement;
    SUBMIT_BTN       : HTMLDivElement;
    MSG_INPUT        : HTMLTextAreaElement;
    MSG_CONTAINER    : HTMLDivElement;
    FIREBASE_APP     : FirebaseFunctions;

    constructor(FIREBASE_APP : FirebaseFunctions){
        this.BOARD_OVERLAY = document.getElementById("village-board")   as HTMLDivElement;
        this.HOUSE_IMG = document.getElementById("house1-png")          as HTMLImageElement;
        this.CLOSE_BTN = document.getElementById("board-close")         as HTMLDivElement;
        this.SUBMIT_BTN = document.getElementById("msg-submit")         as HTMLDivElement;
        this.MSG_INPUT = document.getElementById("msg-input")           as HTMLTextAreaElement;
        this.MSG_CONTAINER = document.getElementById("msg-container")   as HTMLDivElement;
    ...
    async loadMessagesFromFirebase(){
        const MESSAGE_INNER_HTML = await this.FIREBASE_APP.downloadData("messages");
        this.MSG_CONTAINER.innerHTML = MESSAGE_INNER_HTML;
    }

    attachEventListeners(){
        const TABLE_IMAGE = document.getElementById("bookOnTable") as HTMLImageElement;
        TABLE_IMAGE.addEventListener("click",()=>{
            this.BOARD_OVERLAY.style.display = "flex";
        });

        this.CLOSE_BTN.addEventListener("click",()=>{
            this.BOARD_OVERLAY.style.display = "none";
        });

        this.SUBMIT_BTN.addEventListener("click",()=>{
            const TEXT = this.MSG_INPUT.value.trim();
            if(TEXT === "") return;

            // 枠の作成
            const NEW_MESSAGE = document.createElement("div");
            NEW_MESSAGE.className = "msg-item";
            ...
            const MINUTES = String(NOW.getMinutes()).padStart(2, '0'); // 1桁の時は0で埋める

            const TIME_STAMP = `${MONTH}月${DATE}日 ${HOURS}時${MINUTES}分`;
            NEW_MESSAGE.innerHTML = `<strong>${TIME_STAMP}</strong><br>${TEXT}`;

            //リストの先頭に追加
            this.MSG_CONTAINER.prepend(NEW_MESSAGE);

            // 入力欄を空にしてフォーカス
            this.MSG_INPUT.value = "";

            this.sendMessagesToFirebase();
        });
    }

    sendMessagesToFirebase(){
        this.FIREBASE_APP.uploadData("messages",this.MSG_CONTAINER.innerHTML)
    }
...
class SnowyEffect{
    ...
    constructor(){
        this.CANVAS = document.getElementById("skyCanvas") as HTMLCanvasElement;
        ...
        this.SNOW_COUNT = 500;
        this.WIND_SPEED = 1.5; // 風
        this.TARGET_DIRECTION  = 1; //１は左方向。　ー１は右方向　０は重力方向
        this.CURRENT_DIRECTION = 1;
        this.FALL_SPEED = 2.0;
        this.LERP_SPEED = 0.01; //変化の滑らかさ（小さいほどゆっくり）

        this.makeSnowParticles();
    ...
    makeSnowParticles(){
        for(let i = 0; i < this.SNOW_COUNT; i++){
            this.snowParticles.push({
                ...
                size    : Math.random() * 4 + 1, // 1px~4pxのランダムな大きさ
                speedX  : this.WIND_SPEED + Math.random(),
                speedY  : this.FALL_SPEED + Math.random(),
    ...
    updateSnow(){
        ...
        this.CTX.clearRect(0, 0, this.width, this.height);//画面を透明に掃除
        ...
        for(let particle of this.snowParticles){
            ...
            // 風の表現
            particle.x += particle.speedX*this.CURRENT_DIRECTION;
            ...
            // 2. 右端に出た時の処理（左に戻す）
            // ここで y を Math.random にしすぎると、上から降る雪の密度が減って見えます
            if (particle.x > this.width) {
                particle.x = -10; 
                // yはそのままにするか、少しランダムにずらす程度が自然
                particle.y = Math.random() * this.height; 
            }

            // 3. 【追加】左端に出た時の処理（右から風が吹いている場合など）
            if (particle.x < -10) {
        ...
        requestAnimationFrame(() => this.updateSnow()); //次のフレーム予約
    }

    windDirection(){
        const CHANGE_CHANCE = Math.random();
        if(CHANGE_CHANCE < 0.01){
           const SECOND_CHANGE_CHANGE = Math.random();
           if(SECOND_CHANGE_CHANGE < 0.3){
                this.WIND_SPEED = Math.random() * 3; 
                this.TARGET_DIRECTION = Math.floor(Math.random()*3) - 1;
        ...
        this.CURRENT_DIRECTION += (this.TARGET_DIRECTION - this.CURRENT_DIRECTION) * this.LERP_SPEED;
                                                                                     //線形補完。なんと、％加算・減算なので無限に目標値に近づくだけで重なることはないらしい。
    }
}
...
class TimeGradation{
    constructor(){
        const CYCLE_DURATION_MS : number = 180;
        const NOW_MS : number = Date.now() / 1000 ; 
        const SYNC_DELAY  : string = `-${NOW_MS % CYCLE_DURATION_MS}s`;
        
        document.documentElement.style.setProperty('--sync-delay', SYNC_DELAY);
...
class HouseLightControle{
    FIREBASE_APP : FirebaseFunctions;
    LOCAL_ID     : string;

    constructor(FIREBASE_APP : FirebaseFunctions){
        this.FIREBASE_APP = FIREBASE_APP;
        const HOUSE1 = document.getElementById("house1-container") as HTMLDivElement;
        const HOUSE2 = document.getElementById("house2-container") as HTMLDivElement;
        const HOUSE3 = document.getElementById("house3-container") as HTMLDivElement;
        const HOUSE4 = document.getElementById("house4-container") as HTMLDivElement;
        const HOUSE5 = document.getElementById("house5-container") as HTMLDivElement;
        ...
        this.LOCAL_ID = this.loadOrProvideLocalID();
        this.sendIamMeadow();
        ...
        setInterval(() => {
            this.init(HOUSES);
            this.adjustHouseYPosition();

        }, 3000);
    }

    loadOrProvideLocalID(): string{
        var localID : string|null = localStorage.getItem("userLocalID");

        if(localID){
            return localID;
        }else{
            localID = new UtilsFunctions().provideRandomID().replace("-","");
    ...
    sendIamMeadow(){
        //const HOUSES_STATUS =  await this.FIREBASE_APP.downloadData("housesStatus");
        const NOW = Date.now();
        this.FIREBASE_APP.uploadData(`accessLogs/${this.LOCAL_ID}`,NOW)
    }

    async init(HOUSES : HTMLDivElement[]){
        const RESULT = await this.countActiveHouse();
        this.activateLightAndSmoke(HOUSES,RESULT);
    }

    async countActiveHouse(){
        // ① 全データのダウンロード
        const allLogs = await this.FIREBASE_APP.downloadData("accessLogs");
        
        if (!allLogs) return { active: 0, smoking: 0 };

        const now = Date.now();
        const ONE_HOUR = 1000 * 60 * 60;
        const FIVE_HOURS = ONE_HOUR * 3;

        // const DEBUG_ACTIVE_MS = 60*1000;
        // const DEBUG_SMOKE_MS = 120*1000;

        // 各ユーザーのタイムスタンプだけを取り出す
        const timestamps: number[] = Object.values(allLogs);

        // ② 条件（ア）：1時間以内（灯りと煙）
        const activeCount = timestamps.filter(ts => {
             return (now - ts) <= ONE_HOUR;
            // return (now - ts) <= DEBUG_ACTIVE_MS;
        }).length-1;

        // ③ 条件（イ）：1時間以上5時間未満（煙のみ）
        const smokingCount = timestamps.filter(ts => {
            const diff = now - ts;
            return diff > ONE_HOUR && diff < FIVE_HOURS;
            //return diff > DEBUG_ACTIVE_MS && diff < DEBUG_SMOKE_MS;
        }).length;

        return {
            active: activeCount,  // 灯りをつける家の数
            smoking: smokingCount // 煙だけ出す家の数
        };
    }

    provideHourlyPriorityList():number[]{//gemini製
        const now = new Date();
        // 種（シード）を作る: 例 "2024-1-1-14"
        const seed = `${now.getFullYear()}-${now.getMonth() + 1}-${now.getDate()}-${now.getHours()}`;
        
        // 文字列(seed)を数値に変換する（簡単なハッシュ関数）
        let hash = 0;
    ...
    activateLightAndSmoke(HOUSES : HTMLDivElement[], RESULT : any){
        const PRIORITY_LIST = this.provideHourlyPriorityList();     
        HOUSES.forEach((houseElement, index) => {
            // この家(index)がリストの何番目にいるか
            const ORDER = PRIORITY_LIST.indexOf(index);
            // 例
            // PRIORITY_LIST = [2,0,4,1,3]　の時
            // HOUSE forEach2番目なら 0となる。
            // 0は優先順位　１番を表す

            const IS_LIGHT_AND_SMOKE = ORDER < RESULT.active;
            // 灯と煙が出る家の数までこの変数にtrueを入れる
            const IS_ONLY_SMOKE = ORDER < (RESULT.active + RESULT.smoking);
            // 煙だけの家の数まで、tureを入れる。

            // クラスの付け外し
         
            const Light_HOUSE_CONTAINER = document.getElementById(`house${index+1}-lightContainer`) as HTMLDivElement;
            const DARK_HOUSE_PNG = document.getElementById(`house${index+1}-png`) as HTMLImageElement;
            // if(IS_LIGHT_AND_SMOKE){
            //     HOUSE_PNG.src = "light_house.png";
            //     houseElement.classList.toggle("is-active",true);
            //     HOUSE_PNG.classList.toggle("is-active",true);
            // }else if(IS_ONLY_SMOKE){
            //     HOUSE_PNG.src = "dark_house.png";
            //     HOUSE_PNG.classList.toggle("is-active",false);
            //     houseElement.classList.toggle("is-active",true);
            // }else{
            //     HOUSE_PNG.src = "dark_house.png";
            //     houseElement.classList.toggle("is-active",false);
            // }

            // HOUSE_PNG.src = IS_LIGHT_AND_SMOKE ? "light_house.png" : "dark_house.png";

            // 2. 明るさとアニメーションの制御
            if (IS_LIGHT_AND_SMOKE) {
                houseElement.classList.toggle("is-active",true);
                DARK_HOUSE_PNG.style.opacity = "0";
                DARK_HOUSE_PNG.style.zIndex  = "1";

                Light_HOUSE_CONTAINER.style.opacity = "1";
                Light_HOUSE_CONTAINER.style.zIndex = "5";
           
            }else if(IS_ONLY_SMOKE){
                houseElement.classList.toggle("is-active",true);
                DARK_HOUSE_PNG.style.opacity = "1";
                DARK_HOUSE_PNG.style.zIndex  = "5";

                Light_HOUSE_CONTAINER.style.opacity = "0";
                Light_HOUSE_CONTAINER.style.zIndex = "1";
    ...
    adjustHouseYPosition(){
        const HOUSE = document.getElementById("house1-png") as HTMLImageElement;
        const HOUSE_HEIGHT = (HOUSE.offsetHeight -5)* -1;

        const HOUSES = document.getElementById("houses") as HTMLDivElement;
...
interface HORSE_ATTRIBUTES {
    topPosition     : number;
    leftPosition    : number;//位置情報

    xDirection      : number;
    yDirection      : number;//歩く向き

    stepDistance    : number;//一歩の距離
    walkChance      : number;//歩く確率
}
class Horse{
    HORSE!           : HTMLImageElement; //後ほど初期化するので「！」つける。


    IS_BEFORE_STOP  : boolean;

    HORSE_ATTRIBUTES! : HORSE_ATTRIBUTES;//後ほど初期化するので「！」つける。

    ID              : number;

    FIREBASE_APP    : FirebaseFunctions;

    constructor(idIndex : number, FIREBASE_APP : FirebaseFunctions, PRELOADER : PreLoader|null){
    
        this.IS_BEFORE_STOP = false;
    ...
    private makeHorseElement(idIndex : number) : HTMLImageElement{
        // const CONTAINER     = document.querySelector(".objects") as HTMLDivElement;
        // const HORSE_IMG     = document.createElement("img") as HTMLImageElement;

        // HORSE_IMG.src = "walk_right.png";
        // HORSE_IMG.id  = `horse${idIndex}`;
        // HORSE_IMG.className = "horse";

        // CONTAINER.appendChild(HORSE_IMG);
        return document.getElementById(`horse${idIndex}`) as HTMLImageElement;
    }
    // private readPosition() : void{
    //     this.HORSE.style.top    = ( ) + "px";
    //     this.HORSE.style.left   = ( ) + "px";

    // }
    private async init(PRELOADER : PreLoader){
        const DATA  = await this.FIREBASE_APP.downloadData(`horse/id${this.ID}`)
        if(PRELOADER){
            PRELOADER.closePreLoader("SelfCloseMode");
        }
        
        this.HORSE              = this.makeHorseElement(DATA.id) as HTMLImageElement;
    ...
    private initHorsePosition(){
        this.HORSE.style.top    = `${this.HORSE_ATTRIBUTES.topPosition}px`;
        this.HORSE.style.left   = `${this.HORSE_ATTRIBUTES.leftPosition}px`;
        this.adjustZindex(this.HORSE_ATTRIBUTES.topPosition);
    }
 


    private walk(imageSrc : string){

        new HtmlFunction().changeImage(this.HORSE,imageSrc);
        
        this.changePosition();
        this.constrolePicSizeAndDirection();

        this.saveCurrentHorseStatusToFirebase();
    }
    ...
    private getCurrenPosition() : POSITION{
        const STYLE                  = window.getComputedStyle(this.HORSE);
        const CURRENT_TOP   : number = parseFloat(STYLE.top)  ;
        const CURRENT_LEFT  : number = parseFloat(STYLE.left) ;

        const RETURN_OBJ    : POSITION = {currentTop : CURRENT_TOP, currentLeft : CURRENT_LEFT};
        return  RETURN_OBJ;
    }
    ...
    private addVhw() : void{
        ...
        // 2. 移動距離（stepDistance）も「画面の何％分か」として定義しておく
        // 例: stepDistance が 1 なら、画面横幅の 1% 分進むという意味
    ...
    private adjustPositionAndDirection(){
        const CURRENT_POSITION : POSITION = this.getCurrenPosition();
    ...
    private adjustPositionAndDirection4Vhw(){
        const CURRENT_POSITION : POSITION = this.getCurrenPosition();
    ...
    private changeDirection(xy : string, index : number){
        if(xy == "x" && index == 1){
            this.HORSE_ATTRIBUTES.xDirection = 1;
            
        }else if(xy == "x" && index == -1){
            this.HORSE_ATTRIBUTES.xDirection = -1;
  
        }else if(xy == "y" && index == 1){
            this.HORSE_ATTRIBUTES.yDirection = 1;
    ...
    private constrolePicSizeAndDirection() : void{
        const SCALE = this.calcScale();
        
        if      (this.HORSE_ATTRIBUTES.xDirection ==  1 && this.HORSE_ATTRIBUTES.yDirection ==  1){
            this.HORSE.style.transform = `scale(${SCALE},${SCALE})`;
        }else if(this.HORSE_ATTRIBUTES.xDirection ==  1 && this.HORSE_ATTRIBUTES.yDirection == -1){
            this.HORSE.style.transform = `scale(${SCALE},${SCALE})`;
        }else if(this.HORSE_ATTRIBUTES.xDirection == -1 && this.HORSE_ATTRIBUTES.yDirection ==  1){
            this.HORSE.style.transform = `scale(-${SCALE},${SCALE})`;
        }else if(this.HORSE_ATTRIBUTES.xDirection == -1 && this.HORSE_ATTRIBUTES.yDirection ==  -1){
    ...
    private calcScale(){ // GEMINI製のSCALE計算
        const POSITION      = this.getCurrenPosition();
        const TOP_MIN       = -60;
        const TOP_MAX       = 500;
        const SCALE_MIN     = 1.0;
    ...
    private vhwcalcScale(){ // GEMINI製のSCALE計算
        const pos = this.getCurrenPosition();
        const vh = window.innerHeight / 100;

        // 設定（あなたの可動範囲に合わせて調整）
        const START_VH = 40;  // 最も奥
        const END_VH = 80;    // 最も手前
        ...
        // --- 【解決策】平方根（ルート）を使う ---
        // これにより、0.1歩動いただけでも、スケールに大きな変化が生まれます
    ...
    private saveCurrentHorseStatusToFirebase(){
        const POSITION = this.getCurrenPosition();
        const STATUS  : HORSE_ATTRIBUTES= {
            xDirection  : this.HORSE_ATTRIBUTES.xDirection,
            yDirection  : this.HORSE_ATTRIBUTES.yDirection,
            walkChance  : this.HORSE_ATTRIBUTES.walkChance,
            stepDistance: this.HORSE_ATTRIBUTES.stepDistance,
            topPosition : POSITION.currentTop,
            leftPosition: POSITION.currentLeft
        };       
        const DATA   = {id: this.ID, horseAttribute : STATUS};  
        this.FIREBASE_APP.uploadData(`horse/id${this.ID}`,DATA);                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        

    }

    private returnNextImageSrc() : string{
        ...
        switch(this.HORSE.src.split("/").pop()){
            ...
            case "walk_middle1.png":
                return "walk_left.png";

            case "walk_left.png":
                return "walk_middle2.png"

            case "walk_middle2.png":
                return "walk_right.png";

            default:
                // console.log(`in returnNextImageSrc, ERROR : src is ${this.HORSE.src}`);
                return "";
        }
    }

    private decideWalkByChance() : boolean{
        var willWalk : boolean;
        const NOW  = new Date().getTime();
        const TIME_VALUE = NOW % 100;
        if(this.IS_BEFORE_STOP){
            willWalk = TIME_VALUE < (this.HORSE_ATTRIBUTES.walkChance*100);
            
        }else{
            willWalk = TIME_VALUE > 100 - (this.HORSE_ATTRIBUTES.walkChance*100);
           
        }

        willWalk ? this.IS_BEFORE_STOP = false : this.IS_BEFORE_STOP = true;

        return willWalk;
    }
}

interface SoundEventItem {
    object: HTMLElement ;
    sound: HTMLAudioElement[];
}
class SoundEffect{
        currentPlayingSound             : HTMLAudioElement | null = null;
    constructor(){
        // sound定義
    ...
    initEvents(){
        const EVENTS_MAP  = this.provideSoundWithObjRecords();
        EVENTS_MAP.forEach(item => {
            this.attachEventsListener(item.object, item.sound);
        })
    }
    provideSoundWithObjRecords() : SoundEventItem[] {
        // ＝＝＝＝＝＝＝＝＝＝
        // ＝＝　object定義　＝
        // ＝＝＝＝＝＝＝＝＝＝

        const DARK_HOUSE1   = document.getElementById("house1-container")       as HTMLDivElement;
        const DARK_HOUSE2   = document.getElementById("house2-container")       as HTMLDivElement;
        const DARK_HOUSE3   = document.getElementById("house3-container")       as HTMLDivElement;
        const DARK_HOUSE4   = document.getElementById("house4-container")       as HTMLDivElement;
        const DARK_HOUSE5   = document.getElementById("house5-container")       as HTMLDivElement;

        const LIGHT_HOUSE1  = document.getElementById("house1-lightContainer")  as HTMLDivElement;
        const LIGHT_HOUSE2  = document.getElementById("house2-lightContainer")  as HTMLDivElement;
        const LIGHT_HOUSE3  = document.getElementById("house3-lightContainer")  as HTMLDivElement;
        const LIGHT_HOUSE4  = document.getElementById("house4-lightContainer")  as HTMLDivElement;
        const LIGHT_HOUSE5  = document.getElementById("house5-lightContainer")  as HTMLDivElement;

        const HORSE1        = document.getElementById("horse1")         as HTMLImageElement;
        const HORSE2        = document.getElementById("horse2")         as HTMLImageElement;
        const HORSE3        = document.getElementById("horse3")         as HTMLImageElement;
        const HORSE4        = document.getElementById("horse4")         as HTMLImageElement;
        const HORSE5        = document.getElementById("horse5")         as HTMLImageElement;

        const BOOK_ON_TABLE = document.getElementById("bookOnTable")    as HTMLImageElement;
    
        const MEADOW        = document.getElementById("meadow")         as HTMLDivElement;
        ...
        // ＝＝　　sound定義　＝＝
        // ＝＝＝＝＝＝＝＝＝＝＝＝
        const DARK_HOUSE_SOUNDS: HTMLAudioElement[] = [
                                                            new Audio("knock1.mp3"),
                                                            new Audio("knock2.mp3"),
                                                            new Audio("knock3.mp3"),
                                                            new Audio("bell1.mp3")
                                                        ];
        const LIGHT_HOUSE_SOUNDS: HTMLAudioElement[] = [
                                                        new Audio("up.mp3"),
                                                        new Audio("eat.mp3"),
                                                        new Audio("close1.mp3"),
                                                        new Audio("bell1.mp3"),
                                                        new Audio("knock1.mp3"),
                                                        new Audio("knock2.mp3"),
                                                        new Audio("knock3.mp3"),
                                                        new Audio("knock4.mp3"),
                                                        new Audio("cartain.mp3"),
                                                        new Audio("wash.mp3"),
                                                        new Audio("oven.mp3"),
                                                        new Audio("ketle.mp3"),
                                                        new Audio("cut1.mp3"),
                                                        ...
                                                        new Audio("cook1.mp3"),
                                                        new Audio("cook2.mp3"),
                                                        new Audio("toilet.mp3"),
                                                        new Audio("clean1.mp3"),
                                                        new Audio("clean2.mp3"),
                                                        new Audio("clean3.mp3"),
                                                        new Audio("clock1.mp3"),
                                                        new Audio("clock2.mp3"),
                                                        new Audio("clock3.mp3")
                                                       
                                                    ];

        const HORSE_SOUNDS: HTMLAudioElement[] = [
                                                    new Audio("cry1.mp3"),
                                                    new Audio("cry2.mp3"),
                                                    new Audio("cry3.mp3"),
                                                    new Audio("cry4.mp3"),
        ...
        const BOOK_ON_TABLE_SOUNDS  : HTMLAudioElement[]= [

                                                            ];

        const MEADOW_SOUNDS        : HTMLAudioElement[]= [
                                                            new Audio("wind1.mp3"),
                                                            new Audio("wind2.mp3"),
                                                            new Audio("wind3.mp3"),
                                                            new Audio("wind4.mp3"),
        ...
        const SKY_SOUNDS        : HTMLAudioElement[]= [
                                                            new Audio("prelude.mp3")
                                                         ];

        return [
            {object : DARK_HOUSE1        , sound : DARK_HOUSE_SOUNDS          },
            {object : DARK_HOUSE2        , sound : DARK_HOUSE_SOUNDS          },
            {object : DARK_HOUSE3        , sound : DARK_HOUSE_SOUNDS          },
            {object : DARK_HOUSE4        , sound : DARK_HOUSE_SOUNDS          },
            {object : DARK_HOUSE5        , sound : DARK_HOUSE_SOUNDS          },

            {object : LIGHT_HOUSE1        , sound : LIGHT_HOUSE_SOUNDS          },
            {object : LIGHT_HOUSE2        , sound : LIGHT_HOUSE_SOUNDS          },
            {object : LIGHT_HOUSE3        , sound : LIGHT_HOUSE_SOUNDS          },
            {object : LIGHT_HOUSE4        , sound : LIGHT_HOUSE_SOUNDS          },
            {object : LIGHT_HOUSE5        , sound : LIGHT_HOUSE_SOUNDS          },

            {object : HORSE1        , sound : HORSE_SOUNDS          },
            {object : HORSE2        , sound : HORSE_SOUNDS          },
            {object : HORSE3        , sound : HORSE_SOUNDS          },
            {object : HORSE4        , sound : HORSE_SOUNDS          },
            {object : HORSE5        , sound : HORSE_SOUNDS          },

            {object : BOOK_ON_TABLE , sound : BOOK_ON_TABLE_SOUNDS  },
            {object : MEADOW        , sound : MEADOW_SOUNDS         },

            {object : SKY           , sound : SKY_SOUNDS}
        ] 
    }

    attachEventsListener(object : HTMLElement,  sound : HTMLAudioElement[]): void{
  
        object.addEventListener("click",()=>{
            // this.stopCurrentSound();
            // console.log(object);
            this.setNextPlayingSound(sound);
            this.playSound();
        })
    }
    ...
    setNextPlayingSound(sound : HTMLAudioElement[]) : void {
        const RANDOM = Math.floor(Math.random()*sound.length);
        this.currentPlayingSound = sound[RANDOM];
        // console.log(`rnd is ${RANDOM} and sound is ${sound[RANDOM]}`)
        // console.log(sound);
    ...
    playSound() : void {
        ...
        // console.log(this.currentPlayingSound);
    }
...
class omikuzi{
    ...
    constructor(){
        this.HORSE = document.getElementById("horseGod") as HTMLImageElement;
        this.SKY   = document.getElementById("sky")      as HTMLDivElement;
        this.bottomPersentage = 100;
        this.opacity = 0;
        this.lerp = 0.001;
        this.intervalID = "";
    ...
    loadOrSetUserID(){
        var id : number = Number(localStorage.getItem("userOmikuziID"));

        if(id){
            return id;
        }else{
            id = Math.floor(Math.random()*1000);
            localStorage.setItem("userOmikuziID",id.toString())
            return id;
        }
    }
    getOmikuziNumber(userOmikuziID : number){
        const NOW  = new Date();
        const OMIKUZI_RAW_NUM = userOmikuziID + userOmikuziID*(NOW.getMonth()+1)*NOW.getDate();
        return OMIKUZI_RAW_NUM % 6
    }

    initHorseDisplay(){
        this.HORSE.style.bottom = "100%";
        this.HORSE.style.opacity = "0";
    }

    attachEvent(){
  
        this.SKY.addEventListener("click",()=>{
            this.initHorseDisplay();
            this.setHorseOmikuzi();
            if(this.intervalID){
    ...
    setHorseOmikuzi(){
        const USER_ID : number = this.loadOrSetUserID();
        this.HORSE.src = this.OMIKUZI_IMGS[this.getOmikuziNumber(USER_ID)];

    }

    horseCome(){
        this.lerp = 0.002;
        this.bottomPersentage += (0 - this.bottomPersentage) * this.lerp;
        this.HORSE.style.bottom =  `${this.bottomPersentage}%`

        this.opacity += (0.5 - this.opacity) * this.lerp ;
        this.HORSE.style.opacity = `${this.opacity}`;

        if(this.bottomPersentage < 0.5){
            clearInterval(this.intervalID);
            this.intervalID =setInterval(() => {
                                                    this.horseReturn();
                                                }, (10));
        }
        // console.log(`bottom is ${this.bottomPersentage}% and opacity is ${this.opacity}`)
    }

    horseReturn(){
        this.lerp = 0.002;
        this.bottomPersentage += (100 - this.bottomPersentage) * this.lerp;
        this.HORSE.style.bottom =  `${this.bottomPersentage}%`

        this.opacity += (0 - this.opacity) * this.lerp ;
        this.HORSE.style.opacity = `${this.opacity}`;

        if(this.bottomPersentage > 99){
            clearInterval(this.intervalID);
...
class NewYearCard{
    ...
    constructor(){
        const CONFIG        = {
                                apiKey: "AIzaSyBeduEZZ8Wh4K7U2gtAOTAI-R2cCFnn0f0",
                                authDomain: "newyearcard-f6a48.firebaseapp.com",
                                projectId: "newyearcard-f6a48",
                                storageBucket: "newyearcard-f6a48.firebasestorage.app",
                                messagingSenderId: "1078911605170",
                                appId: "1:1078911605170:web:f708d3c698bc10f0baacab",
                                measurementId: "G-PMCLZZLX00"
                                };
        ...
        PRELOADER.boundBalls("SelfCloseMode",null,0,{
            DISPLAY_CONTENT : "年賀状を読み込んでいます・・・",BACKGROUND_COLOR : RGB,FONT_COLOR : new UtilsFunctions().chooseSuitableFontColor(RGB)
        });

        const HORSE  :Horse =  new Horse(1, this.FIREBASE_APP,null);
        const HORSE2 :Horse =  new Horse(2, this.FIREBASE_APP,null);
        const HORSE3 :Horse =  new Horse(3, this.FIREBASE_APP,null);
        const HORSE4 :Horse =  new Horse(4, this.FIREBASE_APP,null);
        const HORSE5 :Horse =  new Horse(5, this.FIREBASE_APP,PRELOADER);

       
        const UTILSFUNC : UtilsFunctions = new UtilsFunctions();

        const HOUSES_CONTROLER = new HouseLightControle(this.FIREBASE_APP);
        ...
        new Notebook(this.FIREBASE_APP);
...
const APP : NewYearCard = new NewYearCard();
